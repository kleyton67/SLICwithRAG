...
def limpar_diretorio(dir):
    for (raiz, diretorio, arquivo) in os.walk(dir):
...
def updateParametros(val):
    ...
    limpar_diretorio(pasta_banco_imagens)
...
def classify(event):
	...
	#Retorno de imagem com limites entre regioes marcadas em destaque.
        ...
        out = graph.rag_boundary(labels, ncut, connectivity=2)
